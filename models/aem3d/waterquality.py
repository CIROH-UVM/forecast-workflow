...
def gencarbonfile(whichbay):
    #
    #   Carbon data time series
    #
    THEBAY = whichbay

    with open('TEMPLATES/wq_c.template.txt', 'r') as file:
        template = Template(file.read())

    pathedfile = os.path.join(THEBAY.infile_dir, 'WQ_C.dat')
    with open(pathedfile, 'w') as output_file:

        # remember generated bay files
        THEBAY.addfile(fname='WQ_C.dat')
...
def gensilicafile(whichbay):
    ...
    with open('TEMPLATES/wq_si.template.txt', 'r') as file:
        template = Template(file.read())

    pathedfile = os.path.join(THEBAY.infile_dir, 'WQ_Si.dat')
    with open(pathedfile, 'w') as output_file:

        # remember generated bay files
        THEBAY.addfile(fname='WQ_Si.dat')
...
def genwqfiles (whichbay):
  ...
  #
  phytofiles = glob.glob('TEMPLATES/*Phyto.template')
  # Loop through list of Phyto Templates, replacing $year with specific year
  for thisfile in phytofiles:        # for each filename

    with open(thisfile,'r') as file:
      phytoStr = file.read()
    template = Template(phytoStr)
    ...
    logger.info('Writing Phyto File, '+pathedfile)
  ...
  for baysource in THEBAY.sourcelist :
    ...
    if cqVersion == 'Clelia':
        ...
        phosdf['TP'].loc[flowdf['msflow'] < 0.1] = 0.0
    else:
