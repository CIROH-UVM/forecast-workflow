...
def gencntlfile(theBay, settings):
	...
	### NOTE: adding 1 to the iterations so that the last forecast timestamp is
	###       6/8 00:00 instead of 6/7 23:55 (given forecast start is 6/1 00:00)
	###       each AEM3D iteration is 5 minutes, which is how long we want to 
	###       extend the forecast to get to a full 7 days
	iterations = int((settings['forecast_end'] - simstart).total_seconds() / AEM3D_DEL_T) + 1
